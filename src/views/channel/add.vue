            ...
            <!-- 模型弹窗 -->
                <el-dialog title="选择模型模板"  :visible.sync="tplContentVisbile" width="60%">
                      <div v-for="(item,index) in tplAll" :key="item.id" class="model-select">
                            <el-checkbox   :true-label="item.id" false-label=" " v-model="info.modelIds[index]"
                            >{{item.name}}</el-checkbox>
...
export default {
  ...
  methods: {
    ...
    getAllList() {
      ...
      if (this.params.parentId != "") {//栏目数据回填
        axios
          .post(api.channelGet, { id: this.params.parentId })
          .then(res => {
        ...
        .then(
          axios.spread((channels, models, tpls, groups,tplAll) => {
            ...
            this.tplAll=tplAll.body;//所有模型
             if(this.params.id==0){  //此处处理模型模版 添加时
     ...
     add(state){//添加栏目  
           let form = this.$refs['form'];
            form.validate((valid) => {//验证方法
                if (valid) {
                    this.loading = true;
                    axios.post(this.$api.channelSave, this.getParams())
                        .then(res => {       
                            if (res.code == "200") {
